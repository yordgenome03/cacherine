...
`peek()` returns the value for a key without removing it from the cache. It
...
invalidate multiple keys. Each present key follows the same Ephemeral FIFO
consumption effects as the equivalent single-key operation.
