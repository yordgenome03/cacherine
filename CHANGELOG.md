...
- Documented that a nullable key type (`K?`) is not supported for eviction purposes: `CacheStore.selectVictim()`/`evictOne()` use `null` as both their "no victim" result and a valid `K` value, so an entry actually stored under the literal key `null` can never be selected as a victim — eviction would report nothing evictable for it forever, even though every store handles `null` as an ordinary key otherwise. See `CacheStore.selectVictim()`'s doc comment for the full tradeoff analysis (an unambiguous fix was considered — a record-wrapped result — but not applied, since `TTLFifoStore` is bound by the pre-existing, unbounded `SimpleTTLCacheInterface`/`ThreadSafeTTLCacheInterface` public interfaces this package cannot change without a breaking release). In practice, using a nullable key type with this package is unusual; this is a documented limitation, not a planned fix.
